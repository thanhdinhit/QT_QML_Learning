...
void SerialOBD::ConnectToSerialPort()
{
    ...
    if(m_serial.open(QIODevice::ReadWrite)){
        ...
        while(data.isEmpty()){
            ...
            if(regExOk.match(data).hasMatch()){
                while(!data.isEmpty())
                    RequestClusterData();
            }
            else
...
void SerialOBD::ParseAndReportClusterData(QByteArray data)
{
    ...
    int k = 0;
    QRegularExpression TCodeRegEx(".*43.*");
    QRegularExpression dataTemp(".*\\d:\\s\\w\\w\\s\\w\\w\\s\\w\\w\\s\\w\\w\\s\\w\\w.*");

    if(!dataTemp.match(data).hasMatch() && !TCodeRegEx.match(data).hasMatch())
        data = "";

    if(dataTemp.match(data).hasMatch()){
    ...
    if(m_tCodeCounter == 20 || TCodeRegEx.match(data).hasMatch()){
        int i = 0;
